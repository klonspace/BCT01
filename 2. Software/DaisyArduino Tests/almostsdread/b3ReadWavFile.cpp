...
double b3ReadWavFile::interpolate(double b, unsigned int channel, b3DataSource &dataSource) const {
  // double frame = fmod(0.4 * m_numFrames * b, m_numFrames);
  double frame = m_numFrames * fmod(abs(b) + numBeats, numBeats)/numBeats;
  // double frame = fmod(b, m_numFrames);
  ...
  if (dataType_ == B3_SINT8) {  // signed 8-bit data
    unsigned char buf[4];
