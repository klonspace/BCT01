...
public:
  ...
  unsigned char readRaw8(unsigned char registerAddress) {
    ...
    int err = wireChannel->endTransmission(false);
    if (err != 0) {
      Serial.print("error during readraw8 : ");
      Serial.println(err);
    }
  ...
  void setZeroPosition() {
    this->setZeroPosition(this->getRawAngle());
  }
  ...
  void setResolution(unsigned int angleSteps) {
    ...
    angleSteps = min(max((int)angleSteps, 8), 2048);
