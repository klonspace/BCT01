...
static void VerbCallback(float** in, float** out, size_t size)
{
    ...
    if(patch.encoder.RisingEdge()
       || patch.gate_input[DaisyPatch::GateInput::GATE_IN_1].Trig())
    {
        gRisingEdge = true;
    }
    ...
    if(!gRisingEdge)
    {
        ...
        if((prevCtrlVal[1] < (ctrlVal[1] - delta))
           || (prevCtrlVal[1] > (ctrlVal[1] + delta)))
        {
            prevCtrlVal[1] = ctrlVal[1];
        }

        if((prevCtrlVal[2] < (ctrlVal[2] - delta))
           || (prevCtrlVal[2] > (ctrlVal[2] + delta)))
        {
            prevCtrlVal[2] = ctrlVal[2];
        }
        if((prevCtrlVal[3] < (ctrlVal[3] - delta))
           || (prevCtrlVal[3] > (ctrlVal[3] + delta)))
        {
        ...
        for(size_t i = 0; i < size; i++)
        {
            ...
            wavFileReaders[selected_file_index].tick(
                0,
                &wavTickers[selected_file_index],
                dataSources[selected_file_index],
                speed);
            ...
            out[0][i]
                = wavTickers[selected_file_index].lastFrame_[0] * ctrlVal[1];
            out[1][i]
                = wavTickers[selected_file_index].lastFrame_[1] * ctrlVal[1];
            //out[0][i] = 0;
            //out[1][i] = 0;
...
void UpdateOled()
{
    ...
    else
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            const char* selected
                = ((i + start) == selected_file_index) ? ">" : " ";
            if(i < file_cnt_)
            {
                test::sprintf(
                    buf, "%s%s", selected, file_info_[i + start].name);
            }
            else
            {
        ...
        float frac
            = nf ? 100. * wavTickers[selected_file_index].time_ / float(nf) : 0;

        test::sprintf(buf,
                      "s:%.2f,%3.0f\%",
                      speed,
                      frac); //wavTickers[selected_file_index].time_);
                       ...
                       == FR_OK)
                    {
                        ...
                        if(memoryBuffer)
                        {
                               ...
                               == FR_OK)
                            {
                                ...
                                num_frames
                                    = wavFileReaders[file_cnt_].getNumFrames();
                                wavFileReaders[file_cnt_].resize();
                                wavTickers[file_cnt_]
                                    = wavFileReaders[file_cnt_].createWavTicker(
                                        samplerate);
    ...
    while(1)
    {
        ...
        if(gRisingEdge)
        {
            if(wavTickers[selected_file_index].time_ < 0)
                wavTickers[selected_file_index].time_
                    = (double)(wavFileReaders[selected_file_index]
                                   .getNumFrames()
                               - 1.0);
            else
